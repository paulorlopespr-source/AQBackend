...
def _quarter_parts(line:float)->tuple[float,float]:return math.floor(line*2)/2,math.ceil(line*2)/2


def settle_total(side:str,value:float,line:float,odd:float)->LegSettlement:
    if int(round(abs(line)*100))%50!=25:return _simple_total(side,value,line,odd)
    a_line,b_line=_quarter_parts(line);a=_simple_total(side,value,a_line,odd);b=_simple_total(side,value,b_line,odd);mult=((a.multiplier or 0)+(b.multiplier or 0))/2
    if a.status==b.status:return LegSettlement(a.status,mult,f"Linha asiática {line}")
    statuses={a.status,b.status}
    if statuses=={LegStatus.WIN.value,LegStatus.PUSH.value}:return LegSettlement(LegStatus.HALF_WIN.value,mult,f"Meia vitória em {line}")
    if statuses=={LegStatus.LOSS.value,LegStatus.PUSH.value}:return LegSettlement(LegStatus.HALF_LOSS.value,mult,f"Meia perda em {line}")
...
def settle_leg(market_id:str,side:str,line:float|None,odd:float,match:dict)->LegSettlement:
    home=match.get("home_goals");away=match.get("away_goals");mid=market_id.lower();side_upper=side.upper()
    ...
    if mid.startswith("dupla_chance") or mid.startswith("double_chance"):
        home_win=home>away;draw=home==away;away_win=away>home;won=(side_upper=="1X" and(home_win or draw))or(side_upper=="X2" and(away_win or draw))or(side_upper=="12" and not draw)
        return LegSettlement(LegStatus.WIN.value if won else LegStatus.LOSS.value,odd if won else 0.0,f"Dupla chance {side_upper}")

    if "ht" in mid and ("goal" in mid or "gol" in mid):
        hh=match.get("halftime_home_goals");ha=match.get("halftime_away_goals")
        if hh is None or ha is None or line is None:return LegSettlement(LegStatus.WAITING_STATS.value,None,"Placar do intervalo ainda indisponível")
        return settle_total(side,float(hh+ha),line,odd)
    if "ht" in mid and ("corner" in mid or "escante" in mid):
        corners=match.get("halftime_corners")
        if corners is None or line is None:return LegSettlement(LegStatus.WAITING_STATS.value,None,"Escanteios HT não disponíveis com segurança na fonte")
