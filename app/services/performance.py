...
from app.services.audit import recommendation_payload, write_audit
...
def _metric(rows):
    ...
    wins=sum(1 for r in settled if r.result in {"GREEN","WIN"});losses=sum(1 for r in settled if r.result in {"RED","LOSS"})
...
def log_recommendations(payload: dict, analysis: dict) -> int:
    ...
    entries=analysis.get("recommended_entries") or [];created=0
    with SessionLocal() as db:
        for item in entries:
            market=str(item.get("market") or "");selection=str(item.get("selection") or "")
            ...
            for source in payload.get("market_probabilities") or []:
                if source.get("market")==market and source.get("selection")==selection:odd=source.get("best_odd");break
            row=AQRecommendation(fixture_id=fixture_id,league=str(payload.get("league") or ""),home_team=str(payload.get("home_team") or ""),away_team=str(payload.get("away_team") or ""),market=market,selection=selection,probability=int(item.get("probability") or 0),confidence=int(item.get("confidence") or payload.get("data_confidence") or 0),risk=str(item.get("risk") or "ALTO"),mode="PRE_LIVE",offered_odd=float(odd) if odd else None,calibration_bucket=bucket_for_probability(int(item.get("probability") or 0)),model_version=MODEL_VERSION)
            db.add(row);db.flush();write_audit(db,"RECOMMENDATION_CREATED","recommendation",str(row.id),"Recomendação AQ registrada",{"input_summary":payload.get("summary",""),**recommendation_payload(row)});created+=1
        db.commit()
    return created
    ...
    with SessionLocal() as db:
        ...
        for row in rows:
            ...
            try:
                ...
                odd=row.offered_odd or (100.0/max(row.probability,1));settlement=settle_leg(market_id,side,line,odd,match)
                ...
                row.settled_profit_unit=round((settlement.multiplier or 0)-1,4);row.settled_at=datetime.now(timezone.utc);write_audit(db,"RECOMMENDATION_SETTLED","recommendation",str(row.id),f"Recomendação liquidada como {row.result}",recommendation_payload(row));settled+=1
            except Exception as exc:
                write_audit(db,"RECOMMENDATION_SYNC_ERROR","recommendation",str(row.id),str(exc),recommendation_payload(row));errors+=1
        db.commit()
...
def recommendation_report():
    ...
    for r in rows:by_market[r.market].append(r);by_league[r.league].append(r);by_mode[r.mode].append(r);by_bucket[bucket_for_probability(r.probability)].append(r);by_model[getattr(r,"model_version",MODEL_VERSION)].append(r)
    calibration=[]
...
def calibration_weights():
    ...
    for item in report["calibration"]:
        gap=item["gap"];weights[item["bucket"]]=1.0 if item.get("samples",0)<30 else round(max(0.85,min(1.02,1+gap/200)),3)
